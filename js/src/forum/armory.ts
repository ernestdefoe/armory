...
export function mountArmory(root: HTMLElement, csrf: string) {
  ...
  function headerHtml(c: any) {
    ...
    const arena = OWN && ARENA_OK ? '<button type="button" id="ar-arena" class="Button Button--primary ar-syncbtn"><i class="fas fa-dungeon"></i> Add to Arena</button>' : '';
    return '<div class="ar-head"><div><h1 class="ar-name" style="color:' + cc(c.class) + '">' + esc(c.name) + '</h1><div class="ar-titleline">' + title + '</div></div><div class="ar-ilvl"><b>' + (c.item_level || 0) + '</b><span>Item level</span></div>' + rp + arena + sync + '</div>';
  }
  ...
  function renderHero() {
    ...
    const ab = detail.querySelector('#ar-arena') as HTMLElement;
    if (ab) ab.onclick = () => {
      const orig = ab.innerHTML;
      ab.textContent = 'Importing…';
      postAction('/armory/character/' + D.character.id + '/arena').then((r: any) => {
        if (r && r.ok) { ab.innerHTML = '<i class="fas fa-check"></i> Deck built (' + r.cards + ' cards)'; }
        else { ab.innerHTML = orig; window.alert((r && r.error) || 'Arena import failed.'); }
      });
    };
  }
