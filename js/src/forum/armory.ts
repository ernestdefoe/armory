...
const QUAL: Record<string, string> = {
  POOR: '#9d9d9d', COMMON: '#cfcfcf', UNCOMMON: '#1eff00', RARE: '#0070dd', EPIC: '#a335ee',
...
export function mountArmory(root: HTMLElement, csrf: string) {
  ...
  function attachTips() {
    ...
    detail.querySelectorAll('.ar-slot[data-i]').forEach((el) => {
      ...
      if (!it) return;
      el.addEventListener('mouseenter', () => { t.innerHTML = buildTip(it); t.style.display = 'block'; });
